...
import os
# In[2]:
class GetFeaturesTestingInFiles :

    p = ""
    path = "../testing_data/input/images/" + p
    path_output = "../testing_data/input/" 
    
    path_write_file = "testing_features_data.txt"
    windows_size_rows = 0
    windows_size_cols = 0
    ...
    def __init__(self, path):
        self.p = path
        self.path = "../testing_data/input/images/" + path +"/"
        self.path_output = "../testing_data/input/" 
        self.path_write_file = "testing_features_data.txt"
    ...
    def write_samples(self, path_write_file_lable, path_write_file_features):
        ...
        for index in range(len(self.samples_contour)):
            a.append(self.samples_contour[index])
            a.append(self.samples_inner[index])
            a.append(self.samples_outer[index]) 
                     
        self.shuffle_samples(a)         
...
#         print(self.path+filename_rgb)
        ...
        self.process_get_sample(img_rgb, img_gray, self.windows_size_rows, self.windows_size_cols, self.path + "traning_data_label_" + filename + ".txt",self.path_output + "testing_data_features_" + filename + ".txt")
        print("finished " +self.path_output + "testing_data_features_" + filename + ".txt"+ "\n>>>>>>>>>>")
    ...
    def process(self) :
#         file_write = open(self.path_output+self.path_write_file, "w")
